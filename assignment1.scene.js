...
class SceneNode {
    ...
    calculateWorldTransformation() {
        ...
        // Initialize an Identity Matrix (The starting point of the world)
        let world = mat4.create()

        // Get the list of transformations: [Local, Parent, ..., Root]
        let transformations = this.getTransformationHierarchy([])

        // Iterate Backwards: From Root (End of array) -> Local (Start of array)
        // We do this because the hierarchy is effectively: Root * Parent * Child
        for (let i = transformations.length - 1; i >= 0; i--) {
            // Multiply the current world matrix by the next transformation in the chain
            // world = world * transformations[i]
            mat4.multiply(world, world, transformations[i])
        }
